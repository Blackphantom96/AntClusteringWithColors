...
public class ColorFunction implements Function<int[]> {
	...
	public int probDeposit(int x, int y, Particle<int[]> p) { // Eq 5.15
		...
		return (int) (fx < k2 ? 2.0 * fx + 0.5 : 1.0) *100;
	}

	@Override
	public double itemPerceivedFraction(int x, int y, int r, Particle<int[]> p) { // FIXME verificar si esto es
																					// necesario .
	...
	public double itemAverageSimilarity(int x, int y, int r, Particle<int[]> p) { // Eq 5.13 o mirar Eq 5.16 en caso de
		...
		for (int i = x - r - 1; i < x + r; i++) {
			for (int j = y - r - 1; j < y + r; j++) {
				if (0 <= i && i < maxX && 0 <= j && j < maxY ) {
					ColorParticle tempParticle = (ColorParticle) CoreFactoryCreator.getFactory().getInstance().getParticles()[i][j];
